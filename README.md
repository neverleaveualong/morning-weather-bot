...
Open-Meteo API와 Telegram Bot API를 사용해 매일 아침 날씨를 텔레그램으로 보내는 서버리스 알림 봇입니다.

서버를 계속 켜둘 필요 없이 GitHub Actions cron으로 정해진 시간에 실행됩니다. 기본 구성은 원주와 서울의 현재 날씨, 오늘 시간별 날씨, 내일 오전/오후 요약을 전송합니다.
...
2. Normalize current, hourly, and tomorrow forecast values.
3. Format a Telegram-friendly message.
4. Send it through Telegram Bot API.
